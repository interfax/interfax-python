from interfax.files import Files, File
...
class TestFile(object):
    ...
    def test_with_large_file(self, fake):
        ...
        assert f.mime_type == None
        assert f.body == None
        assert f.file_tuple() == (None, '', None, {'Content-Location': document.uri})
        ...
        while cursor < len(data):
            ...
            calls.append(call.upload(document.id, 
                cursor, cursor + len(chunk) - 1, chunk))
            cursor += len(chunk)
